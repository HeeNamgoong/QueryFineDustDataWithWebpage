...
const char* mqtt_server = "192.168.232.84"; //mqtt 서버 주소(라즈베리파이에서 ifconfig로 inet 주소 확인)**꼭 수정!!!!!!!!!!!!!
const char* clientName = "0603"; // client 이름(생일 추천드려요~)
//192.168.232.218
...
void reconnect() {
  ...
  while (!client.connected())
  {
    ...
    if (client.connect(clientName))
    {
      ...
      client.subscribe("led_test"); // led 토픽 구독
    } 
    else 
    {
